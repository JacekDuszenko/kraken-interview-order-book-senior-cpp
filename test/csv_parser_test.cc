...
TEST(CsvParser, NewOrder) {
  ...
  EXPECT_EQ(order->Stringified(), "NewOrder[user_id: 1, ticker: IBM, price: 10, quantity: 100, order_side: BID, user_order_id: 1]");
}
...
TEST(CsvParser, CancelOrder) {
  ...
  EXPECT_EQ(order->Stringified(), "CancelOrder[user_id: 2, user_order_id: 102]");
}
